...
def _heuristic_ddg(wt_aa: str, mut_aa: str) -> float:
    """Heuristic delta-delta-G (kcal/mol) for binding - negative = weaker binding."""
...
def get_ppi_ddg_predictions(gene: str, interactors: list, wt_aa: str, pos: int, mut_aa: str) -> list:
    """Get delta-delta-G (wild vs mutant) for each selected PPI. Calculates immediately."""
    from config import PPI_PDB_COMPLEXES
    results = []
    for ip in interactors:
        ...
        if isinstance(complex_info, dict):
            rng = complex_info.get("uniprot_range", (0, 0)) or (0, 0)
            low, high = (rng[0], rng[1]) if len(rng) >= 2 else (0, 0)
        else:
            low, high = 0, 0
        in_range = bool(complex_info and low <= pos <= high)
